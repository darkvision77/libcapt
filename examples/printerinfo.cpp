#include "libcapt/Protocol/ExtendedStatus.hpp"
#include "libcapt/Protocol/PageParams.hpp"
#include "libcapt/Protocol/Protocol.hpp"
#include <cassert>
#include <cstdio>
#include <iostream>
#include <print>
#include <fstream>
#include <vector>

using namespace Capt;

static void printPrinterInfo(Protocol::PrinterInfo info) {
    std::println("Printer Info:");
    std::println("    Type = {}", info.Type);
    std::println("    Version = {}.{:02}", info.VersionMajor, info.VersionMinor);
    std::println("    BlockSize = {}", info.BlockSize);
    std::println("    Buffers = {}", info.Buffers);
    std::println("    RAM Total = {:.02f} MB", (static_cast<double>(info.BlockSize * info.Buffers) / 1024.0) / 1024.0);
}

static std::size_t fillBuffer(std::iostream& printerStream, std::size_t batchSize, std::size_t thresh = 0) {
    std::size_t count = 0;
    std::vector<uint8_t> buffer(batchSize, 0xff);
    while (true) {
        if (thresh != 0 && count >= thresh && batchSize != 1) {
            batchSize = 1;
            buffer.resize(batchSize);
        }
        Protocol::BasicStatus bs = Protocol::PCR_GET_BASIC_STATUS(printerStream);
        if ((bs & Protocol::BasicStatus::IM_DATA_BUSY) != 0) {
            break;
        }
        if ((bs & Protocol::BasicStatus::CMD_BUSY) != 0) {
            std::println("\nfillBuffer error: CMD_BUSY");
            break;
        }
        if ((bs & Protocol::BasicStatus::ERROR_BIT) != 0) {
            std::println("\nfillBuffer error: ERROR_BIT");
            break;
        }
        Protocol::IC_VIDEO_DATA(printerStream, buffer);
        count += buffer.size();
        std::print("\033[2K\rFilling: {}", count);
        std::fflush(stdout);
    }
    std::println();
    return count;
}

int main(int argc, char* argv[]) {
    if (argc != 2) {
        std::println("Usage: {} printerdev", argv[0]);
        return 1;
    }
    std::fstream printerStream(argv[1], std::ios_base::in | std::ios_base::out | std::ios_base::binary);
    if (!printerStream.is_open()) {
        std::println("Failed to open printer stream");
        return 1;
    }

    Protocol::PrinterInfo info = Protocol::PC_GET_PRINTER_INFO(printerStream);
    printPrinterInfo(info);

    Protocol::ExtendedStatus status = Protocol::PC_GET_EXTENDED_STATUS(printerStream);
    if (status.PaperAvailableBits != 0) {
        std::println("Video buffer size detection is not available: remove paper first");
        return 0;
    }

    if ((status.Basic & Protocol::BasicStatus::IM_DATA_BUSY) != 0) {
        std::println("Video buffer size detection error: IM_DATA_BUSY");
        return 1;
    }

    assert(Protocol::PC_RESERVE_UNIT(printerStream) == 0);
    std::println("Unit reserved");

    Protocol::PCR_CLEAR_ERROR(printerStream);
    Protocol::PCR_DISCARD_DATA(printerStream);
    Protocol::PCR_GO_ONLINE(printerStream, 0);

    Protocol::IC_BEGIN_DATA(printerStream);

    Protocol::PageParams dummyParams{
        .PaperSize = 0x09,
        .TonerDensity = 0x3f,
        .Mode = 0,
        .Resolution = Protocol::ResolutionIdx::RES_600,
        .SmoothEnable = true,
        .TonerSaving = false,
        .MarginLeft = 1,
        .MarginTop = 1,
        .ImageLineSize = 620,
        .ImageLines = 7014,
        .PaperWidth = 4960,
        .PaperHeight = 7014,
    };
    Protocol::IC_BEGIN_PAGE(printerStream, dummyParams);

    std::println("Filling video buffer: pass 1...");
    const std::size_t batchSize = 6144;
    const std::size_t thresh = 16 * 1024;
    std::size_t buffSize = fillBuffer(printerStream, batchSize);
    assert(buffSize > thresh);

    std::println("Filling video buffer: pass 2...");
    Protocol::PCR_DISCARD_DATA(printerStream);
    buffSize = fillBuffer(printerStream, batchSize, buffSize - thresh);

    std::println("Video buffer size: {}", buffSize);
    return 0;
}
